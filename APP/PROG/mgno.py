...
def rotate_func(kk):
    pdf_in = open(r"%s\mgno1.pdf"%kk,'rb')
    ...
    pdf_out = open(r"%s\mgno11.pdf"%kk, 'wb')
    pdf_writer.write(pdf_out)
    pdf_out.close()
    pdf_in.close() 

    import os
    os.remove(r"%s\mgno1.pdf"%kk)
...
def MGNO(request,files1):
    ...
    def Folder_creation(cppp):
        ...
        outp = Scheduler.objects.filter(PROGRAM_NAME='MGNO').values()
        res = []
        otp = []
