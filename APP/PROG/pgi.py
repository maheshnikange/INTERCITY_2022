...
from ..STD_FILES.logfile import Log_File
...
def PGI(request,files1):
    ...
    def Folder_creation(cppp):
        ...
        outp = Scheduler.objects.filter(PROGRAM_NAME='PGI').values()
        res = []
        otp = []
    ...
    for i in range(0,total_clients):
        ...
        pdf.ln(4)
        pdf.set_font('Times','',9.06)
        pdf.cell(10,88,txt ='Jika tuan / puan memerlukan penjelasan  selanjutnya berhubung butir / butir yang dinyatakan di hadapan, sila hubungi kami di talian 03-21623222') 
        pdf.ln(1)
        ...
        pdf.image(r'E:\IWOC\APP SCRIPT\PGI\CYCLE\PGI\DATA\BSN_Reminder-02 - Copy.jpg', 145,146,40)

        pdf.ln(1)
        # pdf.image('E:\IWOC\SOURCE\sampleqcode.PNG',40,210,60)
        new_code  = 'b' + str(count) 
        pdf.image(r'E:\IWOC\EXTRA\barcode_fo\%s.png'%(new_code),35,210,40)
    ...
    # lastrecord=data['acc_no'].iat[-1]
    # print(lastrecord,'------------------------------last')
    today = date.today()    
    user = DetailActivity.objects.last()
    first_record = data['acc_no'][0]
    last_record = data['acc_no'].iat[-1]
    ffname = 'pgi'
