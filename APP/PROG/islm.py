...
def rotate_func(kk):
    pdf_in = open(r"%s\ISLM1.pdf"%kk, 'rb')
    ...
    pdf_out = open(r"%s\ISLM00.pdf"%kk, 'wb')
    pdf_writer.write(pdf_out)
    pdf_out.close()
    pdf_in.close() 

    import os
    os.remove(r"%s\ISLM1.pdf"%kk)
...
def ISLM(request,files1):
    ...
    def Folder_creation(cppp):
        ...
        outp = Scheduler.objects.filter(PROGRAM_NAME='ISLM').values()
        res = []
    ...
    # pdf.output('E:\IWOC\APP SCRIPT\ISLM\CYCLE\ILS\DATA\islm1.pdf')
        pdf.output(r"%s\ISLM1.pdf"%kk)
        if i == total_lines-2:
            rotate_func(bk)

# pdf_in = open(r"%s\ISLM11.pdf"%kk ,'rb')
# pdf_reader = PyPDF2.PdfFileReader(pdf_in)
# pdf_writer = PyPDF2.PdfFileWriter()
...
# pdf_out = open(r"%s\ISLM11.pdf"%kk, 'wb')
# pdf_writer.write(pdf_out)
# pdf_out.close()
# pdf_in.close()    