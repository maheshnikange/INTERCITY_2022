...
def REMINDER2(request,files1):
    ...
    def Folder_creation(cppp):
        ...
        outp = Scheduler.objects.filter(PROGRAM_NAME='REMINDER2').values()
        res = []
        otp = []
    ...
    for i in range(0,Total_accounts):
        ...
        pdf.image(r'E:\IWOC\APP SCRIPT\REMINDER2\CYCLE\REMINDER2\DATA\BSN_Reminder-02 - Copy.jpg', 145,146,40)

        pdf.ln(1)
        new_code  = 'b' + str(count)
        pdf.cell(11)
        pdf.image(r'E:\IWOC\EXTRA\barcode_for_rem2\%s.png'%(new_code),35,212,50)
    ...
    # data = History.objects.create(Date = str(today),Start = str(start_time),Finish=str(end_time),User = str(user) ,Product = 'KKK', File= str(files1),Status = 'Enable',Log = str(count),ActiveCount = str(Total_accounts),Page = str(totalpages),Impression  = 'DONE',filenamepdf = 'REMINDER2.pdf',totalaccountspdf = str(Total_accounts),totalpagespdf =str(totalpages), firstrecordpdf = str(str(df['ILOM_OLD_SEQUENCE'][0])),lastrecordpdf= str(df['ILOM_OLD_SEQUENCE'].iat[-1]), programnamepdf = 'REMINDER2')                                                                                                                                                                                                                        
    # Log_File(path11)
